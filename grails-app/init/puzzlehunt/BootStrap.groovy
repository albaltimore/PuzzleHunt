...
class BootStrap {
    ...
    def loadFromPath() {
        ...
        config.puzzles.each {
            puzzles[it.id] = new Puzzle(xCor: it.xcor, yCor: it.ycor, name: it.name, solution: it.solution, round: rounds[it.round], timeLimit: it.timeLimit ?: null, disableHint: it.disableHint ?: false, statusBoost: it.statusBoost ?: 0, requiredCount: it.requiresCount ?: 0, isFinal: it.isFinal ?: false)
        ...
        config.resources.each {
            resources[it.id] = new Resource(puzzle: puzzles[it.puzzle], filename: it.file, accessor: UUID.randomUUID().toString(), linkUri: it.link, mustSolve: it.mustSolve ?: false, role: it.role ?: null, textContent: it.text ?: null)
        ...
        config.rounds.each {
            ...
            rounds[it.id].gameoverResource = resources[it.gameoverResource]
            rounds[it.id].winningResource = resources[it.winningResource]
        }
